...
# Determine project root directory. Priority: 1) first script argument 2) BASE_DIR env var 3) directory where the script resides
DEFAULT_BASE_DIR="$(cd "$(dirname "$0")" && pwd)"
BASE_DIR="${1:-${BASE_DIR:-$DEFAULT_BASE_DIR}}"

# Utility to run a command and exit on failure with a helpful message
safe_run() {
  "$@"
  local status=$?
  if [ $status -ne 0 ]; then
...
# Function to get theme for each app
get_theme() {
    case "$1" in
        "Campaign Control") echo "green" ;;
        "Content Calendar") echo "orange" ;;
        "Content Engine") echo "purple" ;;
        "Performance") echo "blue" ;;
        "Settings") echo "purple" ;;
    esac
}

# Function to get gradient for theme
get_gradient() {
    case "$1" in
        "orange")
            echo "linear-gradient(to bottom right, rgb(234, 88, 12), rgb(194, 65, 12), rgb(124, 45, 18))"
            ;;
        "green")
            echo "linear-gradient(to bottom right, rgb(34, 197, 94), rgb(21, 128, 61), rgb(20, 83, 45))"
            ;;
        "blue")
            echo "linear-gradient(to bottom right, rgb(37, 99, 235), rgb(29, 78, 216), rgb(30, 64, 175))"
            ;;
        "purple")
            echo "linear-gradient(to bottom right, rgb(147, 51, 234), rgb(126, 34, 206), rgb(88, 28, 135))"
            ;;
    esac
}

# Process each app
for app_name in "Campaign Control" "Content Calendar" "Content Engine" "Performance" "Settings"; do
    ...
    if ! grep -q "background: linear-gradient" "$APP_DIR/src/index.css"; then
        ...
        if [ $? -ne 0 ] || [ ! -s "$tmp_css" ]; then
          echo "Error: failed to update index.css for $app_name (awk)" >&2
          rm -f "$tmp_css"
          exit 1
        fi
