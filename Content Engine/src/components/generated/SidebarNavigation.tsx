...
const SidebarNavigation: React.FC = () => {
  ...
  return <aside className={`${isCollapsed ? 'w-20' : 'w-80'} flex flex-col relative transition-all duration-300 ease-in-out`}>
      {/* Green glassmorphic overlay for Content Engine */}
      <div className="absolute inset-0 bg-gradient-to-b from-green-600/30 via-green-700/40 to-green-800/50 backdrop-blur-md border-r border-green-400/20" />
      
      {/* Content on top of glassmorphic background */}
      <div className="relative z-10 flex flex-col h-full">
        {/* Header with collapse toggle */}
        <div className="p-6 border-b border-green-400/20" style={{
        height: "185px",
        ...
        <div className="p-6">
          <motion.button className={`${isCollapsed ? 'w-12 h-12' : 'w-16 h-16'} bg-gradient-to-r from-green-500 to-emerald-600 rounded-full flex items-center justify-center text-white font-bold ${isCollapsed ? 'text-sm' : 'text-lg'} shadow-2xl mx-auto transition-all duration-300`} whileHover={{
          scale: 1.05
        }} whileTap={{
          scale: 0.95
        }} style={{
