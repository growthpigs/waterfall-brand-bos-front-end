...
import { Brain, TrendingUp, MessageSquare, FileText, Activity, Users, Calendar, Target } from 'lucide-react';
...
const DashboardCard: React.FC<DashboardCardProps> = ({
  ...
  }} className="bg-purple-900/40 backdrop-blur-lg rounded-2xl p-6 border border-purple-400/30 shadow-xl hover:bg-purple-900/50 transition-all duration-300" style={{
    borderWidth: "1.11111px",
    borderStyle: "solid",
    borderColor: "oklab(0.999994 0.0000455678 0.0000200868 / 0.2)",
    borderRadius: "20px",
    boxShadow: "0 4px 6px -1px rgb(0 0 0 / 0.09), 0 2px 4px -2px rgb(0 0 0 / 0.09)"
  }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {activeProjects.map((project, index) => <div key={index} className="bg-purple-900/30 backdrop-blur-sm rounded-xl p-4 border border-purple-400/20">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {contentTemplates.map((template, index) => <div key={index} className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/10 text-center">
  ...
  return <motion.div initial={{
    ...
    display: "block"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {metrics.map((metric, index) => <div key={index} className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/10">
...
const BrandBOSDashboard: React.FC<BrandBOSDashboardProps> = ({ onNavigate }) => {
  return <div className="min-h-screen w-full flex bg-gradient-to-br from-purple-600 via-purple-700 to-indigo-800">
      ...
      <main className="flex-1 flex flex-col relative min-h-screen">
        <div className="flex-1 pl-4 pr-4 md:pl-0 md:pr-6 lg:pr-8 pt-6 sm:pt-8 pb-6 sm:pb-8 overflow-auto">
          ...
          duration: 0.6
            ...
            <p className="text-xl text-white/80 font-light drop-shadow-sm text-left" style={{
            fontSize: "22px"
          }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <DashboardCard title="Latest CIA Intelligence" number={12} subtitle="Fresh insights from your latest analysis" icon={Brain} delay={0.1} />
            <DashboardCard title="Viral Opportunities" number={8} subtitle="Trending topics ready for content creation" icon={TrendingUp} delay={0.2} />
            <DashboardCard title="Brand Voice Guidelines" number={5} subtitle="Your brand personality and messaging framework" icon={MessageSquare} delay={0.3} />
          </div>
