...
const BrandIntelligencePage: React.FC<BrandIntelligencePageProps> = ({ onNavigate }) => {
  ...
  return (
    <ProfessionalLayout
      theme="blue"
      sidebar={
        <ProfessionalSidebarNavigation onNavigate={onNavigate} activePageId="intelligence" />
      }
    >
      {/* Main Content Area */}
      {/* Main Content */}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                  <div className="lg:col-span-2">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-green-400"
                    />
                  </div>
                  <div>
                    <button
                      onClick={handleAnalyzeNow}
                      className="w-full h-full bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg transition-colors flex items-center justify-center min-h-[42px]"
                    >
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-green-400 appearance-none"
                      >
              ...
              <div className="bg-white/5 rounded-xl p-4 mb-6 border border-blue-400/30">
                ...
                <div className="space-y-4">
                  {opportunities.map(opportunity => (
                    <div
                      key={opportunity.id}
                      className={`p-4 rounded-lg border transition-all ${selectedOpportunity === opportunity.id ? 'bg-blue-500/20 border-blue-400' : 'bg-white/5 border-white/20 hover:bg-white/10'}`}
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center mb-2">
                            <span className="text-lg mr-2">
                              {opportunity.id === 'option-a'
                                ? '🔥'
                                : opportunity.id === 'option-b'
                                  ? '📈'
                                  : '💡'}
                            </span>
                            <span className="text-white font-semibold">
                              Option {opportunity.id.split('-')[1].toUpperCase()} (Score:{' '}
                              {opportunity.score})
                            </span>
                            <span className="ml-2 text-white"> - {opportunity.title}</span>
                          </div>
                          <p className="text-white/70 text-sm ml-6">"{opportunity.description}"</p>
                        </div>
                        <div className="flex gap-2 ml-4">
                          <button
                            onClick={() => handleSelectOpportunity(opportunity.id)}
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${selectedOpportunity === opportunity.id ? 'bg-blue-500 text-white' : 'bg-white/10 hover:bg-white/20 text-white'}`}
                          >
              ...
              <div className="flex items-center justify-between text-sm">
                <div className="flex items-center space-x-4">
                  <span className="text-white/80 flex items-center">
                    <Clock className="w-4 h-4 mr-1" />⏰ Next Analysis: Monday 9:00 AM
                  </span>
                  <span className="text-yellow-400">Auto-select in: 18h 23m</span>
                </div>
                <span className="text-green-400 flex items-center">
                  <CheckCircle className="w-4 h-4 mr-1" />✅ Last Week: Generated 14 content pieces
                  from Option A
                </span>
              </div>
            </div>
              ...
              <div className="bg-white/5 rounded-xl p-4 mb-6 border border-orange-400/30">
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                    ...
                    <span className="text-green-400 flex items-center">
                      <CheckCircle className="w-4 h-4 mr-1" />✅ Complete (July 1)
                    </span>
                  </div>
                  <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                    <span className="text-white flex items-center">🏢 Competitor Intelligence</span>
                    <span className="text-yellow-400 flex items-center">
                      <Clock className="w-4 h-4 mr-1" />⏳ In Progress (Phase 3)
                    </span>
                  </div>
                  <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                    <span className="text-white flex items-center">
                  ...
                  <div className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                    <span className="text-white flex items-center">📈 Performance Correlation</span>
                    <span className="text-blue-400 flex items-center">
                      <Calendar className="w-4 h-4 mr-1" />
                      📅 Scheduled (July 15)
                    </span>
