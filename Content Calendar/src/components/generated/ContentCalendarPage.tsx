  ...
  return <div className="flex h-screen w-full overflow-hidden">
      ...
      <main className="flex-1 flex relative min-h-screen">
              ...
              <div className="bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 p-6 shadow-xl">
                <div className="grid grid-cols-8 gap-4">
                  ...
                  <div className="space-y-4">
                    ...
                    {timeSlots.map(slot => <div key={slot.hour} className="h-16 flex flex-col items-center justify-center">
                  ...
                  {weekData.map((day, dayIndex) => <div key={day.date} className="space-y-4">
                      {/* Day Header */}
                      <div className={`h-12 flex flex-col items-center justify-center rounded-lg ${day.isToday ? 'bg-cyan-500/30 border border-cyan-400/50' : 'bg-white/5'}`}>
                        <span className="text-white text-sm font-medium">{day.dayName}</span>
                        <span className={`text-xs ${day.isToday ? 'text-cyan-200' : 'text-white/60'}`}>
                          {day.dayNumber}
                        </span>
                      </div>
                        ...
                        scale: 1
                            ...
                            className: "w-3 h-3 text-white"
                                ...
                                <div className="text-white/70 text-xs flex items-center justify-between">
                                  ...
                                  {contentForSlot.cluster && <span className="text-xs bg-white/20 px-1 rounded">
                ...
                <div className="space-y-4">
                  ...
                  cluster: 'Customer Success'
                      ...
                      <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-3 ${item.color === 'blue' ? 'bg-blue-400' : item.color === 'yellow' ? 'bg-yellow-400' : item.color === 'green' ? 'bg-green-400' : item.color === 'cyan' ? 'bg-cyan-400' : 'bg-pink-400'}`} />
                        <div>
        ...
        <div className="fixed right-6 top-1/2 transform -translate-y-1/2 z-50">
          {!chatOpen ? <motion.button onClick={() => setChatOpen(true)} className="w-14 h-14 bg-gradient-to-r from-[#06B6D4] to-[#0F766E] rounded-full flex items-center justify-center text-white shadow-2xl" whileHover={{
          scale: 1.05
        }} whileTap={{
          scale: 0.95
        }}>
                  ...
                  <div className="flex-1 p-4 space-y-3 overflow-y-auto h-64">
                    <div className="bg-white/5 rounded-lg p-3">
                      <div className="text-white/80 text-sm">
                        <strong>Cluster Assistant:</strong> I can help you optimize content clusters, analyze authority building sequences, or suggest CIA-powered timing strategies. What would you like to know about your content clusters?
                      </div>
                    </div>
                  </div>
